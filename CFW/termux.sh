...
# Build
build() {
    if command -v warp &> /dev/null || command -v m &> /dev/null; then
    ...
    if git clone https://github.com/FarhadElahi/test.git &&
        cd wireguard-go &&
        go build main.go &&
        chmod +x main &&
...
install() {
    ...
    if wget https://github.com/FarhadElahi/test/raw/main/warp-android-arm64.b6d64e.zip &&
...
install_arm() {
    ...
    WARP_URL="https://github.com/FarhadElahi/test/raw/main/warp-linux-arm64.b6d64e.zip"

    if wget "$WARP_URL" &&
        unzip "warp-linux-arm64.b6d64e.zip" &&
        chmod +x warp &&
        cp warp "$PREFIX/bin/m" &&
        cp warp "$PREFIX/bin/warp"; then
        rm "README.md" "LICENSE" "warp-linux-arm64.b6d64e.zip"
        echo "================================================"
        echo -e "${green}Warp installed successfully.${rest}"
        socks
    else
...
socks() {
   ...
   echo "================================================"
   echo -e "${yellow}To run again, type:${green} warp ${rest}or${green} m ${rest}or${green} ./warp${rest}"
   echo "================================================"
...
uninstall() {
    ...
    if [ -f "$warp" ]; then
        rm -rf "$directory" "$PREFIX/bin/m" "wa.py" "$PREFIX/bin/warp" "$home/wgcf-profile.ini" "$home/warp" "$home/stuff" "$home/wgcf-identity.json" > /dev/null 2>&1
